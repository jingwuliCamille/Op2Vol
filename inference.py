...
from load_data1 import LIDC_IDRI_flow,Chaos,Sliver,Chaos1,Dircadb
...
device = torch.device('cuda:3' if torch.cuda.is_available() else 'cpu')#`设置使用的设备为GPU（如果有）或CPU。
...
net.to(device)
prediction = []
output_path = '/home/zhangwei/ljw/OpticalandAffinity/result/opticalNet/3Dircadb/gallbladder'

model_checkpoint =  '/home/zhangwei/ljw/OpticalandAffinity/checkpoint6/BestModel_11_0.5116727766251365.pth'
root_data = '/raid/Data/lijingwu/3Dircadb/normalized/gallbladder'
# test_dataset = Sliver(root=root_data, load=True, training=False)#加载用于测试的数据集，同样使用LIDC_IDRI_unet类。
test_dataset = Dircadb(root=root_data, load=True, training=False)#加载用于测试的数据集，同样使用LIDC_IDRI_unet类。
# test_dataset = LIDC_IDRI_flow(root=root_data, load=True, training=False)#加载用于测试的数据集，同样使用LIDC_IDRI_unet类。
        ...
        #将 backward_images 和 forward_images 分别存储在名为 images 的列表中，将 backward_masks 和 forward_masks 分别存储在名为 masks 的列表中。
            ...
            #将图像和掩码添加到指定索引的列表中，以便后续的可视化和评估。
                ...
                #从输入数据中提取当前时间步的掩码，并将其移动到指定的设备上。
                    ...
                    flows_f, segs_f, fp2, fp1, flows_b, segs_b,occlusion = net(img1, img2 , training=False)
                #使用输入图像计算前向和后向光流，以及前向和后向分割分支的输出。此外，这里的net指代的是神经网络模型。
                    ...
                    # print(f"Command took {elapsed_time:.4f} seconds.")

                #使用位移场对图像和掩码进行重采样，以获取在当前时间步的图像中的对应像素位置。
                    loss = net_utils.compute_l1_loss(warped_img1,img2, flows_f, use_mag_loss=False)                   
                #计算前向光流的L1损失。
                ...
                #计算分割结果的Dice系数，并将其添加到指定列表中，以便后续的平均值计算。
                    images_warp[idx].append(img2)
                    real_masks[idx].append(mask2)
                    masks_warp[idx].append(warped_mask1)
                    flows_vis[idx].append(flows_f[0])
                    mask1 = mask2
                    img1 = img2
         
            save_test_result(step, images_warp, real_masks ,masks_warp, flows_vis)
            print("photo_loss",np.mean(photo_loss),"dice:",np.mean(dice))
        return np.mean(photo_loss), np.mean(dice)

def save_test_result(step, images_warp,real_masks, masks_warp, flows_vis):
    ...
    for idx in range(len(images_warp)):
        ...
        for i in range(len(images_warp[idx])):
            ...
            real_mask = net_utils.denorm_mask(torch.squeeze(real_masks[idx][i]).cpu().numpy())
            filename = 'slice-' + str(ap) + '.png' if ap != mid else 'slice-' + str(ap) + '-ori.png'
            cv2.imwrite(os.path.join(path, 'mask', filename), real_mask)
